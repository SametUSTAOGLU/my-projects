...
# Create a function named `home` which uses template file named `main.html`
#  given under `templates` folder,
# send your name as template variable, and assign route of no path ('/')
@app.route("/")
def home():
    return render_template("main.html",name="samet")

# Write a function named `greet` which uses template file named `greet.html`
#  given under `templates` folder. it takes parameters from query string on URL, 
# assign that parameter to the 'user' variable and sent that user name into the html file. 
